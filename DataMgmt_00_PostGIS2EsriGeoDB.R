...
geo_analysis_grid_ak_centroid <- sf::st_read(con, query = "SELECT * FROM base.geo_analysis_grid_ak_centroid") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "base\\geo_analysis_grid_ak_centroid"), data = geo_analysis_grid_ak_centroid, overwrite = TRUE)
...
geo_polar_bear <- sf::st_read(con, query = "SELECT * FROM surv_chess.geo_polar_bear") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_chess\\geo_polar_bear"), data = geo_polar_bear, overwrite = TRUE)
rm(geo_polar_bear)
...
geo_detections_by_frame <- sf::st_read(con, query = "SELECT * FROM surv_jobss.geo_detections_by_frame") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_jobss\\geo_detections_by_frame_j"), data = geo_detections_by_frame, overwrite = TRUE)
...
geo_detections_by_seal <- sf::st_read(con, query = "SELECT * FROM surv_jobss.geo_detections_by_seal") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_jobss\\geo_detections_by_seal_j"), data = geo_detections_by_seal, overwrite = TRUE)
rm(geo_detections_by_seal)

# geo_images_meta
geo_images_meta <- sf::st_read(con, query = "SELECT * FROM surv_jobss.geo_images_meta") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_jobss\\geo_images_meta_j"), data = geo_images_meta, overwrite = TRUE)
rm(geo_images_meta)
...
geo_detections_by_frame <- sf::st_read(con, query = "SELECT * FROM surv_polar_bear.geo_detections_by_frame") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_polar_bear\\geo_detections_by_frame_pb"), data = geo_detections_by_frame, overwrite = TRUE)
rm(geo_detections_by_frame)

# geo_detections_by_seal
geo_detections_by_seal <- sf::st_read(con, query = "SELECT * FROM surv_polar_bear.geo_detections_by_seal") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_polar_bear\\geo_detections_by_seal_pb"), data = geo_detections_by_seal, overwrite = TRUE)
rm(geo_detections_by_seal)

# geo_images_footprint
geo_images_footprint <- sf::st_read(con, query = "SELECT * FROM surv_polar_bear.geo_images_footprint") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_polar_bear\\geo_images_footprint_pb"), data = geo_images_footprint, overwrite = TRUE)
rm(geo_images_footprint)

# geo_images_meta
geo_images_meta <- sf::st_read(con, query = "SELECT * FROM surv_polar_bear.geo_images_meta") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_polar_bear\\geo_images_meta_pb"), data = geo_images_meta, overwrite = TRUE)
rm(geo_images_meta)
...
geo_images_meta <- sf::st_read(con, query = "SELECT * FROM surv_pv_gla.geo_images_meta") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_pv_gla\\geo_images_meta_g"), data = geo_images_meta, overwrite = TRUE)
rm(geo_images_meta)
...
geo_detections_by_frame <- sf::st_read(con, query = "SELECT * FROM surv_test_kotz.geo_detections_by_frame") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_test_kotz\\geo_detections_by_frame_k"), data = geo_detections_by_frame, overwrite = TRUE)
rm(geo_detections_by_frame)

# geo_detections_by_seal
geo_detections_by_seal <- sf::st_read(con, query = "SELECT * FROM surv_test_kotz.geo_detections_by_seal") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_test_kotz\\geo_detections_by_seal_k"), data = geo_detections_by_seal, overwrite = TRUE)
rm(geo_detections_by_seal)

# geo_images_footprint
geo_images_footprint <- sf::st_read(con, query = "SELECT * FROM surv_test_kotz.geo_images_footprint") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_test_kotz\\geo_images_footprint_k"), data = geo_images_footprint, overwrite = TRUE)
rm(geo_images_footprint)

# geo_images_meta
geo_images_meta <- sf::st_read(con, query = "SELECT * FROM surv_test_kotz.geo_images_meta") %>%
  sf::as_Spatial()
arc.write(file.path(fgdb_path, "surv_test_kotz\\geo_images_meta_k"), data = geo_images_meta, overwrite = TRUE)
rm(geo_images_meta)
